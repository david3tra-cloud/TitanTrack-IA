...
const WorkoutLogger: React.FC<WorkoutLoggerProps> = ({ onSave, onCancel, routines }) => {
  ...
  return (
    <div className="bg-black/90 border border-white/10 rounded-[32px] p-6 md:p-8 space-y-6 relative">

      {/* Banner de entrenamiento en curso */}
      {showResumeAlert && (
        <div className="bg-cyan-500/10 border border-cyan-500/40 rounded-2xl p-4 flex flex-col gap-3">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 py-2 bg-fuchsia-500/10 border border-fuchsia-500/30 text-fuchsia-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-fuchsia-500 hover:text-black transition-all"
            >
              Empezar de nuevo
            </button>
          </div>
      ...
      {/* Timer de descanso */}
      {timerSeconds !== null && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 bg-cyan-500 text-black px-6 py-3 rounded-full font-black text-sm uppercase tracking-widest shadow-[0_0_30px_rgba(0,242,255,0.8)]">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black uppercase tracking-tighter text-white italic">Bio Registro</h2>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mt-1">
            {Math.floor(totalSeconds / 60)}m {totalSeconds % 60}s
            <span className="ml-3 text-emerald-400 animate-pulse">● ACTIVE</span>
          </p>
        </div>
        <button
          onClick={onCancel}
      ...
      {showRoutineSelector && (
        <div className="space-y-2">
          <h3 className="text-xs font-black uppercase tracking-widest text-slate-500">Seleccionar Rutina Guardada</h3>
          {routines.map(r => (
            <button
              key={r.id}
            ...
            >
              ...
              <p className="text-[10px] text-slate-500">{r.exercises.length} Ejercicios</p>
            </button>
          ))}
        </div>
      )}
      ...
      {/* Nombre de sesión */}
      <div>
        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">Identificador de Sesión</label>
        <input
          type="text"
          value={name}
      ...
      {exercises.map((ex) => {
        ...
        return (
          <div key={ex.id} className="bg-black/40 border border-white/5 rounded-2xl p-4 space-y-4">
            <div className="flex items-center justify-between gap-2">
              <select
                value={ex.name}
                onChange={(e) => setExercises(exercises.map(item =>
            ...
            {/* Cabecera de tabla */}
            <div className="grid grid-cols-[2rem_1fr_1fr_2.5rem_2rem] gap-1 text-[9px] font-black uppercase tracking-widest text-slate-600 text-center">
              <div>Set</div>
              <div>KG</div>
              <div>Reps</div>
              <div>Status</div>
            ...
            {/* Filas de series */}
            {ex.sets.map((set, idx) => (
              <div key={set.id} className="grid grid-cols-[2rem_1fr_1fr_2.5rem_2rem] gap-1 items-center">
                <div className="text-[10px] font-black text-slate-500 text-center">#{idx + 1}</div>
      ...
      <button
        onClick={addExercise}
        className="w-full py-4 border border-dashed border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-white hover:border-cyan-500/30 transition-all"
      >
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={handleSave}
          className="flex-1 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest text-black hover:shadow-[0_0_30px_rgba(0,242,255,0.5)] transition-all"
        >
